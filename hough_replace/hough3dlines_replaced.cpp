...
int main(int argc, char ** argv) {
  ...
  PointCloud X;
   //FILE* fp1 = fopen("/home/me/Documents/H3D/sp.output_1.txt", "w"); 
 ...
 // fclose(fp1);
  ...
  //hough->fout.open("/home/me/Documents/H3D/sp.output.txt");
  ...
  string filepath_ab;
      ...
      // 创建目录（如果不存在）
      string dirPath = string(cwd) + "\\tempfile";
      CreateDirectoryA(dirPath.c_str(), NULL);
      
      filepath_sp = string(cwd) + "\\tempfile\\sp.output.txt";
      filepath_ab = string(cwd) + "\\tempfile\\ab.output.txt";
      ...
      // 创建目录（如果不存在）
      string dirPath = string(cwd) + "/tempfile";
      mkdir(dirPath.c_str(), 0777);
      
      filepath_sp = string(cwd) + "/tempfile/sp.output.txt";
      filepath_ab = string(cwd) + "/tempfile/ab.output.txt";
  #endif
  ...
  if (fp == NULL) {
      cerr << "Error: Cannot open file " << filepath_sp << endl;
      perror("fopen failed");
      return 1;
  }
  ...
  if (fp_ab == NULL) {
      cerr << "Error: Cannot open file " << filepath_ab << endl;
      perror("fopen failed");
      fclose(fp);
      return 1;
  }
  ...
  do {
	...
	// 保存 Y 中的点到文件
	if (!Y.points.empty()) {
	    for (auto& pt : Y.points) {
	    	pt.x += X.shift.x; 
        pt.y += X.shift.y;
        pt.z += X.shift.z;
	     	fprintf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %d %d %d\n",pt.x, pt.y, pt.z, pt.mag, pt.year, pt.month, pt.day, pt.hour, pt.minute, pt.second,0,0,0,Count+1);
	     	//fprintf(fp, "%lf %lf %lf %lf\n",pt.x,pt.y,pt.z,pt.mag);
	     	pt.x -= X.shift.x; 
        pt.y -= X.shift.y;
        pt.z -= X.shift.z;
	    }
