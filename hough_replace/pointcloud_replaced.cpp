...
void PointCloud::shiftToOrigin(){
  ...
  for(size_t i=0; i < points.size(); i++){
    ...
    //printf("%lf %lf %lf %lf\n",points[i].x,points[i].y,points[i].z,points[i].mag);
  }
...
int PointCloud::readFromFile(const char* path, char delim){
  ...
  Vector3d point;
  int n;
  sprintf(formatstring, "%%lf%c%%lf%c%%lf%c%%lf%c%%lf%c%%lf%c%%lf%c%%lf%c%%lf%c%%lf", delim, delim, delim, delim, delim, delim, delim, delim, delim);
  while (fgets(line, 1023, f)) {
    if (line[0] == '#') continue;
    n = sscanf(line, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
           &point.x, &point.y, &point.z, &point.mag,
           &point.year, &point.month, &point.day, &point.hour, &point.minute, &point.second);
    //修改
    if (n != 10) {
      fprintf(stderr, "Error: expected 10 fields, but read %d. Line: %s\n", n, line);
      fclose(f);
      return 2;
    }
